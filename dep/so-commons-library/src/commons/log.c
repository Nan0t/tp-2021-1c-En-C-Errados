...
#define vlog_impl_template(log_function, level_enum) 									\
		void log_function(t_log* logger, const char* message_template, va_list arguments) {	\
			_log_write_in_level(logger, level_enum, message_template, arguments);		\
		}																				\
...
vlog_impl_template(vlog_trace, LOG_LEVEL_TRACE);

log_impl_template(log_debug, LOG_LEVEL_DEBUG);
vlog_impl_template(vlog_debug, LOG_LEVEL_DEBUG);

log_impl_template(log_info, LOG_LEVEL_INFO);
vlog_impl_template(vlog_info, LOG_LEVEL_INFO);

log_impl_template(log_warning, LOG_LEVEL_WARNING);
vlog_impl_template(vlog_warning, LOG_LEVEL_WARNING);

log_impl_template(log_error, LOG_LEVEL_ERROR);
vlog_impl_template(vlog_error, LOG_LEVEL_ERROR);
