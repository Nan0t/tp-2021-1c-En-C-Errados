...
u_msg_movimiento_tripulante_t* u_msg_movimiento_tripulante_deserializar(const u_buffer_t* buffer)
{
    ...
    u_buffer_read(buffer, &pid, sizeof(uint32_t), offset);
    ...
    return u_msg_movimiento_tripulante_crear(pid, tid, pos);
}
...
char* u_msg_movimiento_tripulante_to_string(const u_msg_movimiento_tripulante_t* this)
{
    return string_from_format(
        "MSG_MOVIMIENTO_TRIPULANTE: { PID: %d | TID: %d | Pos_X: %d | Pos_Y: %d }",
        this->pid, this->tid, this->pos.x, this->pos.y
    );
}